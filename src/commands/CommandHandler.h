...
class CommandHandler: public UIController {
  ...
  public:
    ...
    CommandHandler(WINDOW* t_window, VirtualATC &t_atc);
};

CommandHandler::CommandHandler(WINDOW* t_window, VirtualATC &t_atc): UIController(t_window), atc(t_atc)
{
  wrefresh(_window);
}
...
void CommandHandler::quick_setup()
{
    ...
    Traffic t2("BA509", 53.4686977, 1.42833, 35000, 90, 0, 295);
    atc.add_traffic(t1);
    atc.add_traffic(t2);
}
